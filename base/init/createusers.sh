...
#
# If running on localhost, you must edit pg_hba.conf as described in installation docs.
#
...
createuser -e ${PG_HOST:+"--host"} $PG_HOST ${PG_PORT:+"--host"} $PG_PORT --username=postgres --adduser --createrole --createdb --superuser $PG_ADMIN_USER
...
createuser -e ${PG_HOST:+"--host"} $PG_HOST ${PG_PORT:+"--host"} $PG_PORT --username=postgres --no-adduser --no-createrole --no-createdb $PG_WEB_USER
