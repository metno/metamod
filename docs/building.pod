...
=head4 Building for unit testing/deployment
...
This will first download and compile the necessary modules, and put the source
tarballs into the vendor/cache directory to simplify building later.
The version numbers in the snapshot file (for your OS version) will be updated.
