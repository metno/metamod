...
A: You need to block parts of the search interface to robots. Copy the file
app/example/etc/robots.txt to your DocumentRoot directory (changing "metamod"
if you have set LOCAL_URL to something else) and restart Apache.

If you already have a robots.txt, add the following lines to your
existing file (warning: work in progress – always check
app/example/etc/robots.txt for the authorative data):

  User-agent: *
  Disallow: /metamod/search/
  Disallow: /metamod/subscription/
