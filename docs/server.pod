...
=head4 Blocking robots

For better performance you should also block spiders from overloading the search
interface. Copy and optionally modify the file F<app/example/etc/robots.txt>
into your DocumentRoot directory (must be global for the whole site).
