...
For simplicity you may use F<virtualenv.pl> to generate a bash script F<./bin/activate>
...
If for some reason you don't want to run F<activate>, you may instead set the necessary
...
B<Note: The remaining instructions are only required in automated server environments.>
For personal demo use you may safely skip the rest of this document.
