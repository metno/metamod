...
B<Note:> SEARCH and UPLOAD are now integrated with Catalyst and will always be installed (however
...
[This is probably in error. You must run update_target with write permissions to both source and target.]

=head3 Generating target files
...
Install the Perl Catalyst framework onto the target directory. This is done
automatically by Module::Install using the Makefile.PL specified, building
into the blib directory (which is why you need write permission to the source
dir).

=item *

Create an Apache configuration file for the application. This is done by get_httpd_conf.pl,
which creates a file target/etc/httpd.conf with the necessary configuration.
...
=head3 Creating the webrun

After a successful run of the C<update_target.pl> script, the following script
must be run from the target directory as APPLICATION_USER:
...
init.d pointing back to the target directory, as well as a small script for running
