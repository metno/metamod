...
Before you can deploy a METAMOD application you must make a configuration,
...
for the specific application. These files will be placed in in
<configuration dir>/etc, and then symlinked into /etc/apache2 and /etc/init.d.

  install_jobs.sh <config>

=head2 Activating environment (optional)

To make it easier to run commandline scripts you can use the bash script
activate_env that sets the correct environment variables. All commands also run
without this command, but you must then specify the correct paths, configuration
directory and PERL5LIB for all commands.
