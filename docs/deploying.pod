...
placed in in <configuration dir>/etc, and then symlinked into F</etc/apache2> and
F</etc/init.d>.

  $ cd /opt/metno-metamod-2.11 # or elsewhere if installed from source
...
correct executable paths, configuration directory and PERL5LIB for all commands.

=head3 Using activate_env (optional)

For simplicity you may instead use the bash script F<activate_env> that sets the
...
Copy the example config:
  ...
  $ cp -r /opt/metno-metamod-2.11/app/example .

Then edit example/master_config.txt as needed. For local testing, set APPLICATION_USER to your own login.

  $ /opt/metno-metamod-2.11/common/prepare_runtime_env.sh example

  $ export PERL5LIB=/opt/metno-metamod-2.11/common/lib:/opt/metno-metamod-2.11/catalyst/lib:/opt/metno-perl-webdev-ver1/lib/perl5

Try starting Catalyst:

  $ /opt/metno-metamod-2.11/catalyst/script/metamodweb_server.pl -d example

MetamodWeb should now be available on http://localhost:3000/. Stop with ctrl-c.

To install all services:

  $ /opt/metno-metamod-2.11/install_jobs.sh example
