...
If you want to run development versions (recommended until 2.11), also add:
...
=head3 Installing METAMOD and dependencies via apt
  ...
  $ sudo apt-get install metno-metamod-2.11

Alternately you may download the .deb files from repo.met.no and install manually.

  $ sudo dpkg -i metno-metamod-2.11_2.11.0-x_all.deb

where 2.11.0-x is the latest version in the repository.
