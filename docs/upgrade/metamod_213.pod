...
B<NB:> To enable new SRIDs you need to re-initialize the metadata database and
re-import all datasets (preferably by running F<create_and_load_all.sh>).

Also note that since it is not possible to mix static and dynamic maps (and the
...
In case any of these projections are missing you must either install support for them using data from
L<http://spatialreference.org/> or remove them from SRID_ID_COLUMNS.
