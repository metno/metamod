...
OpenLayers.Format.WMC = OpenLayers.Class({
    ...
    contextToMap: function(context, id) {
        var args = {};
        var properties = [
            "maxExtent", "projection",
            "numZoomLevels", "minScale", "maxScale", "units"
        ];
        for(var i=0, len=properties.length; i<len; ++i) {
            if (properties[i] in context) {
                args[ properties[i] ] = context[ properties[i] ];
            }
        }
        if ("maxExtent" in args) {
            args.maxResolution = args.maxExtent.getWidth() / 256;
        }
        args["controls"] = [];
        var map = new OpenLayers.Map(id, args);
    ...
    mapToContext: function(map) {
        ...
        var properties = [
            "maxExtent", "projection", "layers",
            "numZoomLevels", "minScale", "maxScale",
            "controls"
        ];
        for(var i=0, len=properties.length; i<len; ++i) {
            context[ properties[i] ] = map[ properties[i] ];
        }
